...
```{note}
データマイニングは、大量データからパターン・ルール・知識を発見する活動であり、機械学習はその手段のひとつ。
```
...
### 特徴量工学（特徴量エンジニアリング）とその進化史
以上のように、目的に沿った特徴を用意することと、またその特徴からどのようにパターンを抽出するのかを適切に選ぶことが機械学習やデータマイニングにおいては重要である。特に前者の、特定タスクに特化して機能する特徴量を検討し利用することを **{index}`特徴量工学<とくちょうりょうこうがく-特徴量工学>`** もしくは **{index}`特徴量エンジニアリング<とくちょうりょうえんじにありんぐ-特徴量エンジニアリング>`**（**{index}`feature engineering`**）と呼ぶ。本授業では、タスク特化の特徴量も扱うが、汎用性の高い扱い方（＝各種データの取り扱い）についても取り上げる。なお教員の興味の問題で、タスク特化特徴量については自然言語の例示が多く、静止画・音声・動画像あたりのデータはほとんど取り扱わない予定である。
...
```{important}
本講義の主題の一つが「特徴量エンジニアリングの進化史」であり、大凡以下の流れで学ぶことになる。手動→半自動→自動→メタ設計へと進化し続けているが、手動設計がなくなったわけではないことに注意しよう。

- 特徴量エンジニアリングの進化史
  - 【手動】数値・カテゴリを人が変換する: Part 2（第5回〜6回）
  - 【手動】テキストの意味を人が定義・設計する: Part 3前半（第7回〜9回）
  - 【半自動】コーパスからモデルが表現を学習する: word2vec（第10回）
  - 【自動】end-to-endで特徴を内部化する: Transformer/BERT（第11回〜14回）
  - 【メタ設計】入出力・アーキテクチャを設計する: 第15回
```
...
- データセットからパターンを見つける際の問題意識に触れる。（理解は後日で良い）
  - 問題意識の例
    - 「データから知識を得る」とはどういうことか？統計分析や機械学習と何が違うのか？
    - 「機械が見つけたパターン」は本当に知識と呼べるか？人間の解釈・文脈なしに意味をなすか？
    - 「予測精度が高いモデル」と「有用な知識を与えるモデル」は同じか？
    - あなたの専門分野で「知りたいこと」を解くためにデータをどう集めるか？
    - データマイニングと「AIブーム」はどう関係しているか？
- 本授業で扱う範囲を確認し、履修判断のために用いる。
