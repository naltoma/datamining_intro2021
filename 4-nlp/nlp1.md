...
- 参考文献
  ...
  - [Pythonではじめるテキストアナリティクス入門](https://bookclub.kodansha.co.jp/title?code=1000041399)
- 全体の流れ
  - 達成目標
  - 自然言語処理の難しさ
  - アプローチ1: シソーラス
...
局所表現と分散表現の概念図として[言語処理における分散表現学習のフロンティア](https://www.jstage.jst.go.jp/article/jjsai/31/2/31_189/_article/-char/ja/)より抜粋した下図を眺めてみよう。
...
- **相互情報量（Pointwise Mutual Information; PMI）**
  - [Wikipedia: PMI](https://en.wikipedia.org/wiki/Pointwise_mutual_information)
    - 一般的な定義（情報理論、統計学）
      ...
      - 解釈：「2つの事象が同時に現れる確率p(x,y)が、独立だと仮定したときの期待される確率p(x)p(y)よりもどれだけ大きいか」を対数スケールで測る指標。`「強い共起」×「希少性」`によりスコアを付けている。
    ...
    - $pmi(x;y) = log\frac{P(x,y)}{P(x)*P(y)}$：単語xと単語yの相互情報量。
      - PMIが大きくなるのは以下の2ケース。
        - (1) 相互依存（共起）が強い。p(y|x)やp(x|y)が1に近い（≒片方が出たらほぼ確実にもう片方も出る）場合、PMIが大きくなる。
          - 例: New York
        - (2) どちら片方が希少、かつ、一緒に出るときは必ずペア。例えばp(y)が小さいとき、かつ、p(y|x)≈1の場合、PMIが大きくなる。
          - 例: 相互情報量
        - ただし、「めったに見ないのにたまたま一度一緒に出ただけ」でも大きな値となりがちな点に注意。
          - 例: 當間愛晃
  - 例外対応
