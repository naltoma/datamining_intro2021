...
- Level 4: 文書分類せよ。
  - 教師あり学習用にラベル付けされたテキスト文書を準備し、BoW（もしくはBoW+TFIDF）, 共起行列に基づいた特徴ベクトルを用いて分類学習をしてみよう。
    - テキスト文書は任意で構わない。思いつかない場合には、[sklearn.datasets.fetch_20newsgroups](https://scikit-learn.org/0.19/datasets/twenty_newsgroups.html)を利用しよう。なお、全文書を用いなくても良いが、最低限3カテゴリ選択し、カテゴリ毎に100文書以上サンプルを用意すること。
